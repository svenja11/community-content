...
> Uninstall the `hc-utils` package. Otherwise there's a risk, that two DHCP clients will compete over the configuration, which will cause side effects or outages. (See [Hetzner docs](https://docs.hetzner.com/networking/networks/server-configuration#manual-configuration-via-dhcp))
...
Open the [Hetzner Console](https://console.hetzner.com/) and create a new Ubuntu 22.04 server. Add the server to your private Network only. We don't need any public interfaces.
...
If not, you need to use the [VNC console](https://docs.hetzner.com/cloud/servers/getting-started/vnc-console) in the [Hetzner Console](https://console.hetzner.com/).
