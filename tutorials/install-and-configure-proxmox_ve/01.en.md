...
```
Copy and paste the following command to get the latest Proxmox image:
```bash
...
Once the network configuration section has been reached, enter your network details and make sure that "Pin network interface names" is checked. After the installation simply reboot the server.
