...
```
Kopieren Sie den folgenden Befehl um das aktuelle Proxmox-Image herunterzuladen:
```bash
...
MYMAC=$(ip --json a show up | jq -r 'del(.[] | select(.ifname == "lo")) | .[0].address') && qemu-system-x86_64 \
  ...
  -nic none \
  -netdev user,id=n0 \
  -device virtio-net-pci,netdev=n0,mac="$MYMAC" \
...
<img width="800" height="600" alt="image" src="https://github.com/user-attachments/assets/5f6a454e-1c7e-4bf6-8c6c-7678b2b79432" />

#### Konfiguration der Netzwerkschnittstelle

Sobald Sie den Abschnitt zur Netzwerkkonfiguration erreicht haben, geben Sie Ihre Netzwerkdetails ein und stellen Sie sicher, dass „Pin network interface names“ aktiviert ist. Starten Sie den Server nach Abschluss der Installation einfach neu.

<img width="800" height="600" alt="image" src="https://github.com/user-attachments/assets/97fc0ec5-00ed-457a-a10c-2a3fc3f963ea" />
